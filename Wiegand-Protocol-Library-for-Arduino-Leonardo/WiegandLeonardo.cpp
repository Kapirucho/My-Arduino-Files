...
void WIEGAND::begin()
{
	...
	attachInterrupt(0, ReadD0A, FALLING);	// Hardware interrupt - high to low pulse //Pin 3
	attachInterrupt(1, ReadD1A, FALLING);	// Hardware interrupt - high to low pulse //Pin 2
		
	 pinMode(D0PinB, INPUT);					// Set D0 pin as input
	 pinMode(D1PinB, INPUT);					// Set D1 pin as input
	 attachInterrupt(2, ReadD0B, FALLING);	// Hardware interrupt - high to low pulse //Pin 0
	 attachInterrupt(3, ReadD1B, FALLING);	// Hardware interrupt - high to low pulse //Pin 1
}
